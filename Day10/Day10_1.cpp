...
int main(int argc, char *argv[]){
	...
	int ciclos_restantes_instr = 0; // Indica cuantos cilos le quedan a la
									// instruccion
	int X = 1; // Donde se almacena el valor del registro
	int resultado = 0;
	
	while(++ciclo) { // Nunca acaba
		// Se acualiza la suma si es necesario
		if( (ciclo + 20) % 40 == 0) {
			resultado += X * ciclo;
			
			// Nos salimos si ya hemos llegado al ciclo 220
			if (ciclo == 220) break;
		}
