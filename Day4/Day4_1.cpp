...
ships, and so several Elves have been assigned the job of cleaning up sections of the camp. Every section has a unique ID number, and each Elf is assigned a range of section IDs.
...
bool contenido_en(int a, int b, int c, int d) {
	return a >= c && b <= d;
}
...
int main(int argc, char *argv[]){
	...
	do {
		if(contenido_en(prim_inf, prim_sup, seg_inf, seg_sup) || 
		   contenido_en(seg_inf, seg_sup, prim_inf, prim_sup))
			total++;
